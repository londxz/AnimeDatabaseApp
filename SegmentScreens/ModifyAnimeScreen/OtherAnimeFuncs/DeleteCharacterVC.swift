...
class DeleteCharacterVC: UIViewController {
    ...
    private func setTextViews() {
        ...
        descriptionStack = classForGetTextView.getTextView(textField: descriptionTextField, placeholder: "Character description")
        
        view.addSubview(descriptionStack)
        
        NSLayoutConstraint.activate([
            descriptionStack.topAnchor.constraint(equalTo: bannerImage.bottomAnchor, constant: 10),
    ...
    @objc private func deleteCharacter() {
        ...
        if let description = descriptionTextField.text, !description.isEmpty {
            ...
            let result = deleteCharacterByDescription(connection: connection, description: description)
